    ...
    if ~imdb.flip
        for i = 1:length(imdb.image_ids)
            ...
            try
                ...
                warning('%s: %s', imdb.image_ids{i},  e.message);
                voc_rec = [];
            end
            if ~isempty(regions)
...
if isfield(voc_rec, 'objects')
    if exclude_difficult_samples
        ...
        valid_objects = true(length(voc_rec.objects,1));
    end
    common_classes = class_to_id.isKey({voc_rec.objects(:).class})';
    valid_objects = common_classes & valid_objects;
    ...
    try
        ...
        disp({voc_rec.objects(valid_objects).class});
    end
