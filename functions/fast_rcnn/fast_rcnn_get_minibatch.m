...
% --------------------------------------------------------
    ...
    rois_blob = zeros(rois_per_image*num_images, 5, 'single');
    labels_blob = zeros(rois_per_image*num_images, 1, 'single');
    bbox_targets_blob = zeros(rois_per_image*num_images, 4 * (num_classes+1), 'single');
    ...
    for i = 1:num_images
        ...
        assert(size(labels,1) == size(im_rois,1));
        num_rois = size(im_rois, 1);
        % Add to ROIs blob
        ...
        rois_blob(counter+1:counter+num_rois,:) = rois_blob_this_image;

        % Add to labels, bbox targets, and bbox loss blobs
        labels_blob(counter+1:counter+num_rois,:) = labels;
        bbox_targets_blob(counter+1:counter+num_rois,:) = bbox_targets;
        bbox_loss_blob(counter+1:counter+num_rois,:) = bbox_loss;
        counter = counter + num_rois;
    end
    rois_blob = rois_blob(1:counter,:);
    labels_blob = labels_blob(1:counter,:);
    bbox_targets_blob = bbox_targets_blob(1:counter,:);
    bbox_loss_blob = bbox_loss_blob(1:counter,:);
...
%     overlaps = image_roidb(1).max_overlaps;
    ...
    [bbox_targets, bbox_loss_weights] = get_bbox_regression_labels( ...
        image_roidb.bbox_targets(keep_inds, :), num_classes);

end

function [bbox_targets, bbox_loss_weights] = get_bbox_regression_labels(bbox_target_data, num_classes)
