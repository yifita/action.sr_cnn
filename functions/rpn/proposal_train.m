function save_model_path = proposal_train(conf, varargin)
% save_model_path = proposal_train(conf, dataset.imdb_train, roidb_train, varargin)
% --------------------------------------------------------
% Faster R-CNN
...
%% try to find trained model
    ...
    cache_dir = fullfile(conf.cache_root_dir, 'output', 'rpn_cachedir', opts.cache_name, imdbs_name);
...
%% making tran/val data
    ...
    [image_roidb_train, bbox_means, bbox_stds]...
        = proposal_prepare_image_roidb(conf, dataset.imdb_train, dataset.roidb_train);
    fprintf('Done.\n');

    if opts.do_val
        fprintf('Preparing validation data...');
        [image_roidb_val]...
                                = proposal_prepare_image_roidb(conf, dataset.imdb_test, dataset.roidb_test, bbox_means, bbox_stds);
        fprintf('Done.\n');
