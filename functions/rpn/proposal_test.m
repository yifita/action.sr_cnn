...
%% inputs
    ...
    ip.addParamValue('rpn_keep_thres',  0.005,           @isscalar);

    ip.parse(conf, imdb, varargin{:});
    opts = ip.Results;

    cache_dir = fullfile(conf.cache_root_dir, 'output', 'rpn_cachedir', opts.cache_name, imdb.name);
    try
        % try to load cache
...
%% testing
        ...
        for i = 1:num_images
            ...
            fprintf(' time: %.3fs\n', toc(th));
            % for the sake of memory save only boxes with scores > rpn_keep_thres
            % I = (scores > opts.rpn_keep_thres);
            aboxes{i} = [boxes(I,:), scores(I)];
        end
        save(fullfile(cache_dir, ['proposal_boxes_' imdb.name opts.suffix]), 'aboxes');
        diary off;
        caffe.reset_all();
        rng(prev_rng);
    end
